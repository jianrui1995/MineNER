...
import json
def six():
    f_label = open("val_result.txt", "r", encoding="utf8")
    f_word = open("val_wordpiece.txt","r",encoding="utf8")
        ...
        for i in range(len(list_wo)):
            ...
            if list_la[i] ==0 :
                ...
                if list_la[i]%2 == 1:
                    ...
                    end_pos += 1 if list_wo[i] in ["[SPACE]", "[UNK]"] else len(list(list_wo[i]))
                    if type: #有值
                        if type == list_type[(list_la[i]-1)//2]: #如果两个type一样
                            # end_pos += 1 if list_wo[i] in ["[SPACE]", "[UNK]"] else len(list(list_wo[i]))
                            name = name+list_wo[i]
    ...
    f_label.close()
    f_word.close()
    f_submit = open("val_submit.json", "w", encoding="utf8")
